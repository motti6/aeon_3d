...
2. **全国制覇スタンプラリー & Firebase ユーザー進捗管理 (`src/stampManager.js`, `src/firebase.js`)**:
   - **来訪スタンプ記録**: 各店舗カードから「訪問スタンプを押す (行った！)」をワンクリックで記録。訪問日・カスタムメモを保存可能。
   - **進捗率プログレスバー**: 全国601店舗に対する制覇率（%）、訪問店舗数、訪問都道府県数をリアルタイム集計。
   - **「★ 訪問済」専用フィルター**: 過去に行ったイオンだけを一覧および3Dマップ上で瞬時に絞り込み表示。
   - **3Dピンのゴールドバッジ化**: 来訪した店舗のCesium 3Dピンがゴールドクラウン・星付きバッジ（★済）に変化。
   - **Firebase ユーザーID同期 & オフライン対応**:
     - ユーザーID（UID）ごとにFirestoreクラウド同期。
     - 特別な設定を行わなくても、ブラウザローカルストレージで即座に完全動作（ゼロ設定フォールバック）。
     - ユーザーIDのコピー＆別端末への入力で、複数デバイス間でのスタンプ共有・引き継ぎが可能。
     - 画面上のモーダルから任意のFirebaseプロジェクト設定（API Key / Project ID）を後から接続可能。
     - JSON形式でのエクスポート＆インポートによるデータバックアップにも対応。

3. **研究対象都市（札幌・仙台・広島・福岡）および全国の実測フットプリント (`public/data/`)**:
   - 札幌に加え、研究用主要拠点である**仙台・広島・福岡**を含む全国169店舗のOpenStreetMap（OSM）実測建物外形線（`OSM_exact_footprint`）を地表にドレープ描画。国土地理院標準地図の建物枠線と精密に重なります。
   - 各店舗の詳細カードにて「敷地面積」「延床面積」「外周長」「3D推定建物高」「BBox範囲」を表示。
   - **敷地GeoJSONダウンロード & クリップボードコピー**: 空間統計モデルやGIS解析に即座に活用できるGeoJSONエクスポート機能を搭載。

4. **データ収集 & クレンジングパイプライン (`scripts/fetch_aeon.py`, `scripts/verify_cities_osm.py`)**:
   - Overpass APIおよびOSM APIから日本国内のイオン拠点を自動収集・外形線厳密同期。
